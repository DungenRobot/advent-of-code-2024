def target_possible(target_value: int, base: int, remaining: list[int]) -> bool:
    ...
    if target_possible(target_value, base + remaining[0], remaining[1:]):
        return True
    elif target_possible(target_value, base * remaining[0], remaining[1:]):
        return True
    #extra case to check for concatenation
    else:
        base = int(str(base)+str(remaining[0]))
        return target_possible(target_value, base, remaining[1:])
